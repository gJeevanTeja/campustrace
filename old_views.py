...
class ItemListCreateView(generics.ListCreateAPIView):
    ...
    def create(self, request, *args, **kwargs):
        ...
        # Γ£à Notifications run in background thread ΓÇö HTTP 201 returns instantly
        threading.Thread(
    ...
    def _broadcast_new_item(self, item, posted_by):
        ...
        Runs in background thread ΓÇö never blocks HTTP response.
        Γ£à NO email sending here.
        """
        try:
...
class ClaimItemView(APIView):
    def post(self, request, pk):
        ...
        # Γ£à In-app notification only ΓÇö NO email sent on claim
        try:
            from notifications.models import Notification
            Notification.objects.create(
                user=item.user,
                item=item,
                message=f'≡ƒÖï {request.user.name} has claimed your {item.type} item: "{item.title}"',
        ...
        # Γ¥î REMOVED: send_mail() for item claim ΓÇö no email sent here
        # Email is only sent for:
        #   1. OTP login        ΓåÆ users/views.py SendOTPView
        #   2. Password reset   ΓåÆ users/views.py ForgotPasswordView
