...
import { User, Tag, BookOpen, Building, GraduationCap, Phone, Mail, Camera, Image as ImageIcon, Trash2, Key, Bell, FileText, ChevronRight, CheckCircle2, Package, Loader2, LogOut } from 'lucide-react';
...
const Profile = ({ darkMode: dm }) => {
  ...
  const ACCOUNT_FIELDS = [
    { field: 'name', label: 'FULL NAME', value: p.name, icon: <User size={18} />, iconBg: '#eff6ff', iconColor: '#3b82f6', editable: true },
    { field: 'username', label: 'USERNAME', value: p.username, icon: <Tag size={18} />, iconBg: '#f0fdf4', iconColor: '#10b981', editable: true },
    { field: 'section', label: 'SECTION', value: p.section, icon: <BookOpen size={18} />, iconBg: '#fdf2f8', iconColor: '#ec4899', editable: true },
    { field: 'department', label: 'DEPARTMENT', value: p.department, icon: <Building size={18} />, iconBg: '#f5f3ff', iconColor: '#8b5cf6', editable: false },
    { field: 'college_year', label: 'COLLEGE YEAR', value: p.college_year, icon: <GraduationCap size={18} />, iconBg: '#fffbeb', iconColor: '#f59e0b', editable: false },
    { field: 'phone', label: 'PHONE NUMBER', value: p.phone, icon: <Phone size={18} />, iconBg: '#ecfdf5', iconColor: '#059669', editable: true },
    { field: 'email', label: 'UNIVERSITY EMAIL', value: p.email, icon: <Mail size={18} />, iconBg: '#fef3c7', iconColor: '#d97706', editable: false, locked: true },
  ];
  ...
  if (loading) return (
    <div style={{ minHeight: '100vh', background: bg, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      <div style={{ textAlign: 'center' }}><div style={{ display: 'flex', justifyContent: 'center', marginBottom: 12 }}><User size={40} className="animate-spin" /></div><p style={{ color: muted }}>Loading profile...</p></div>
    </div>
  );
          ...
          <div onClick={e => e.stopPropagation()}
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 12 }}>
              ...
              <label style={{ border: `2px dashed ${border}`, borderRadius: 14, padding: '18px 12px', textAlign: 'center', cursor: 'pointer', background: inp, display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
                ...
                <div style={{ marginBottom: 6, color: text }}><ImageIcon size={32} /></div>
      ...
      <div style={{ background: 'linear-gradient(135deg,#2563eb 0%,#7c3aed 100%)', padding: '32px 20px 28px', textAlign: 'center', color: '#fff' }}>
        <div style={{ position: 'relative', display: 'inline-block', marginBottom: 14 }}>
          ...
          <button onClick={() => setShowPhotoMenu(true)} disabled={avatarLoading}
            style={{ position: 'absolute', bottom: 2, right: 2, width: 28, height: 28, borderRadius: '50%', background: '#fff', color: '#1a1a1a', border: 'none', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: '0 2px 6px rgba(0,0,0,.3)' }}>
      ...
      <div style={{ padding: '14px 16px 0' }}>
          ...
          {ACCOUNT_FIELDS.map((row, i) => {
            ...
            return (
              <div key={row.field} style={{ padding: '13px 16px', borderBottom: i < ACCOUNT_FIELDS.length - 1 ? `1px solid ${border}` : 'none', display: 'flex', alignItems: 'center', gap: 12 }}>
                  ...
                  {isEditing ? (
                    <div style={{ display: 'flex', gap: 6, alignItems: 'center' }}>
                      ...
                      <button onClick={cancelEdit}
                        style={{ background: dm ? '#2d2d2d' : '#f1f5f9', color: muted, border: 'none', borderRadius: 7, padding: '7px 9px', cursor: 'pointer', fontSize: 13, flexShrink: 0 }}>✕</button>
                    </div>
                  ) : (
                ...
                {!isEditing && (
                  row.locked
                    ? <span style={{ color: muted, flexShrink: 0, display: 'flex' }}><Key size={16} /></span>
                    : row.editable
                      ? <button onClick={() => startEdit(row.field, row.value)}
                        style={{ background: 'none', border: 'none', cursor: 'pointer', color: muted, flexShrink: 0, padding: 4, display: 'flex' }}>✏️</button>
                      : null
                )}
              </div>
            );
        ...
        <div style={{ background: card, borderRadius: 16, marginBottom: 14, border: `1px solid ${border}`, overflow: 'hidden' }}>
          <div style={{ display: 'flex', borderBottom: `1px solid ${border}` }}>
            {[{ id: 'posted', icon: <Package size={16} />, label: `Posted (${myItems.length})` }, { id: 'claimed', icon: <CheckCircle2 size={16} />, label: `Claimed (${claimedItems.length})` }].map((t) => (
              <button key={t.id} onClick={() => setActiveTab(t.id)}
                style={{
                  flex: 1, padding: '13px 10px', border: 'none', background: 'none', cursor: 'pointer', fontWeight: 700, fontSize: 13,
                  color: activeTab === t.id ? '#2563eb' : muted,
        ...
        <div style={{ background: card, borderRadius: 16, marginBottom: 14, border: `1px solid ${border}`, overflow: 'hidden' }}>
          <button onClick={() => setShowPwdForm(v => !v)}
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
              <div style={{ width: 40, height: 40, borderRadius: 12, background: dm ? '#7c3aed20' : '#f5f3ff', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#7c3aed' }}><Key size={18} /></div>
        ...
        <div style={{ background: card, borderRadius: 16, marginBottom: 14, border: `1px solid ${border}`, overflow: 'hidden' }}>
          {[
            { label: 'Notification Settings', icon: <Bell size={18} />, bg: '#fdf2f8', ic: '#ec4899', to: '/settings' },
            { label: 'Terms & Privacy', icon: <FileText size={18} />, bg: '#f0fdf4', ic: '#10b981', to: '/terms' },
          ].map((link, i) => (
            <button key={link.label} onClick={() => navigate(link.to)}
              ...
              <span style={{ color: muted, display: 'flex' }}><ChevronRight size={16} /></span>
            </button>
          ))}
        </div>
