...
const ItemDetails = ({ darkMode: dm }) => {
  ...
  const handlePayment = async () => {
    const lockedReward = item.matching_lost_item_reward || selectedReward || autoDetails.suggested || 50;
    const priceToUse = item.product_price || customPrice || 1000;
    
    if (!item.matching_lost_item_reward && !lockedReward) return alert("Please specify a reward amount.");
                       ...
                       <div className="space-y-4">
                          {item.contact_phone === 'Locked' ? (
                            <div className="bg-primary/5 dark:bg-primary/10 border border-primary/20 dark:border-primary/30 rounded-3xl p-6 space-y-4 transition-colors">
                               ...
                               {!isCodeVerified ? (
                                   <div className="bg-white dark:bg-slate-900 border border-border dark:border-slate-800 p-5 rounded-2xl space-y-3">
                                      <label className="text-xs font-black text-primary uppercase tracking-wider block text-center">Enter Approval Code</label>
                                      <p className="text-[10px] text-text-secondary dark:text-slate-400 text-center uppercase font-bold">Check your notifications for the 6-digit claim approval code.</p>
                                      <input 
                                         type="text"
                                         value={enteredCode}
                                      ...
                                      <button 
                                         ...
                                         className="w-full bg-slate-900 dark:bg-slate-800 text-white py-3 rounded-xl font-black shadow-lg disabled:opacity-50"
                                      >
                                         Verify Code
                                      </button>
                                   </div>
                                      ...
                                      <div className="space-y-4">
                                         ...
                                         <div className="bg-white dark:bg-slate-900 rounded-2xl p-4 border border-border dark:border-slate-800 space-y-2 transition-colors">
                                            <div className="flex justify-between text-xs">
                                               ...
                                               <span className="text-text-primary dark:text-slate-100 font-black">₹{selectedReward || autoDetails.suggested || 0}</span>
                                            </div>
                                            ...
                                            <div className="pt-2 border-t border-border dark:border-slate-800 flex justify-between text-sm">
                                               ...
                                               <span className="font-black text-primary text-lg">₹{selectedReward || autoDetails.suggested || 0}</span>
                                            </div>
                                         </div>
                                      </div>
                                    )}
