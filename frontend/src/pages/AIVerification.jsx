...
const AIVerification = () => {
    ...
    if (loading) return (
       <div className="flex flex-col items-center justify-center min-h-screen space-y-4 bg-slate-50">
          ...
          <p className="font-black text-text-secondary uppercase tracking-widest text-xs">Booting AI Engine...</p>
       </div>
    );

    if (error) return (
       <div className="flex flex-col items-center justify-center min-h-screen p-6 bg-slate-50 text-center">
          <div className="p-6 bg-white rounded-[32px] shadow-2xl space-y-6 max-w-sm border border-danger/20">
             <div className="w-16 h-16 bg-danger/10 text-danger rounded-2xl flex items-center justify-center mx-auto">
                <ShieldAlert size={32} />
             </div>
             <h2 className="text-xl font-black text-text-primary uppercase tracking-tighter">System Error</h2>
    ...
    return (
        <div className="min-h-screen bg-slate-50 flex flex-col font-sans">
            {/* Nav */}
            <header className="px-6 py-5 flex items-center justify-between bg-white/70 backdrop-blur-xl border-b border-slate-100 sticky top-0 z-50">
                <div className="flex items-center gap-4">
                    <button onClick={() => navigate(`/item/${id}`)} className="p-2.5 bg-slate-100 hover:bg-slate-200 rounded-2xl transition-all">
                ...
                {!finished && !calculating && (
                    <div className="w-full mb-12 space-y-4">
                        <div className="flex justify-between items-end">
                            <span className="text-[10px] font-black uppercase tracking-widest text-text-secondary">Security Protocol Progress</span>
                        ...
                        <div className="flex gap-2.5">
                            {[0, 1, 2, 3, 4].map(s => (
                                <motion.div 
                                  key={s} 
                                  className={`flex-1 h-2 rounded-full transition-all duration-500 ${s < questionIndex ? 'bg-success shadow-lg shadow-success/20' : s === questionIndex ? 'bg-primary shadow-lg shadow-primary/30' : 'bg-slate-200'}`}
                                  initial={false}
                                ...
                                <form onSubmit={handleSubmit} className="space-y-6">
                                    <div className="relative group">
                                        <textarea
                                            ...
                                            className="w-full min-h-[160px] bg-slate-50 border-2 border-slate-100 rounded-3xl p-6 text-base font-medium outline-none focus:border-primary/30 focus:ring-8 focus:ring-primary/5 transition-all resize-none placeholder:text-slate-300"
                                            disabled={submitting}
                                        />
                                        <div className="absolute bottom-4 right-6 text-[10px] font-black uppercase text-slate-300">
                                    ...
                                    <button 
                                      ...
                                      className={`w-full py-5 rounded-[24px] font-black uppercase tracking-widest text-sm flex items-center justify-center gap-3 transition-all ${answer.trim() && !submitting ? 'bg-primary text-white shadow-2xl shadow-primary/30 active:scale-95' : 'bg-slate-100 text-slate-300 pointer-events-none'}`}
                                    >
                                        {submitting ? (
                    ...
                    {calculating && (
                        ...
                        >
                            ...
                            <div className="space-y-2">
                               <h2 className="text-3xl font-black uppercase tracking-tighter text-text-primary">Crunching Data</h2>
                               <p className="text-sm font-medium text-text-secondary max-w-xs mx-auto">AI is comparing your responses with found item metadata and security patterns.</p>
                            </div>
                        </motion.div>
                    )}
                        ...
                        >
                            <PremiumCard className="p-10 text-center space-y-8 overflow-hidden relative" hover={false}>
                                ...
                                <div className="relative z-10 space-y-8">
                                    ...
                                    <div className="space-y-2">
                                       <h2 className="text-4xl font-black text-text-primary tracking-tighter uppercase">Protocol Complete</h2>
                                       <p className="text-text-secondary font-medium">Your claim verification has been finalized.</p>
                                    </div>
                                    ...
                                    <div className="p-8 bg-slate-50 rounded-[32px] border border-slate-100 space-y-4">
                                       ...
                                       <p className="text-xs font-bold text-text-secondary leading-relaxed max-w-[280px] mx-auto uppercase tracking-wide opacity-80">
